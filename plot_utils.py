...
def plot_2d_profile(x_data: np.ndarray, y_data: np.ndarray, z_data: np.ndarray, 
    ...
    # Draw contours? for the 1sigma and 2sigma confidence regions.
    ...
    # Add a star at the best-fit point
    if (z_is_loglike):
        bestfit_indices = np.unravel_index(z_values.argmax(), z_values.shape)
        x_bestfit = x_values[bestfit_indices]
        y_bestfit = y_values[bestfit_indices]
        ax.scatter(x_bestfit, y_bestfit, marker=plot_settings["bestfit_marker"], s=plot_settings["bestfit_marker_size"], c=plot_settings["bestfit_marker_color"],
                   edgecolor=plot_settings["bestfit_marker_edgecolor"], linewidth=plot_settings["bestfit_marker_linewidth"], zorder=100)
